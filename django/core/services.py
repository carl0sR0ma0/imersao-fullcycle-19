...
class VideoService:
    ...
    def process_upload(self, video_id: int, chuck_index: int, chunk: bytes) -> None:
        ...
        with transaction.atomic():
            ...
            if video_media.status == VideoMedia.Status.UPLOADED_STARTED:
                self.storage.storage_chunk(str(video_media.video_path), chuck_index, chunk)
                return
    ...
    def finalize_upload(self, video_id:int, total_chunks:int) -> None:
        video = self.find_video(video_id)
